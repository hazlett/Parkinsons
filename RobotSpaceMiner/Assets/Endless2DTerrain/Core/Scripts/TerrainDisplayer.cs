...
public class TerrainDisplayer : MonoBehaviour {
    ...
    public float terrainLeadAmount;
    ...
    void Update()
    {
        if (!coroutineRunning)
        {
            StartCoroutine(GenerateTerrainCoroutine(terrainLeadAmount));	
        }
    ...
    public void GenerateTerrain(float leadAmount)
    {
        ...
        Vector3 rightSide = new Vector3(MainCamera.transform.position.x, 0, 0);
        Vector3 leftSide = new Vector3(MainCamera.transform.position.x - 15, 0, 0);
